...
class GeraetSteuernService:
    ...
    def execute(self, user, geraetId: int, aktion: str) -> None:
        ...
        if not self.regel_service.darfGesteuertWerden(geraet):
            raise ValueError(
                f"Geraet '{geraet.name}' darf nicht gesteuert werden (Status: {geraet.status.value})"
            )
